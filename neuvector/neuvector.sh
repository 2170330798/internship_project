...
SourceUrl="http://registry.docker.io/neuvector/kubernetes/${VERSION}/neuvector-k8s.yaml"
DestPath="$(pwd)/neuvector-k8s.yaml"

# 创建命名空间和需要的账号:
echo "创建命名空间和sa用户认证证书:"
...
# 修改文件默认镜像
sed -i "s@neuvector/manager:${VERSION}@${DockerHub}/neuvector/manager:${VERSION}@g" ${DestPath}
sed -i "s@neuvector/controller:${VERSION}@${DockerHub}/neuvector/controller:${VERSION}@g" ${DestPath}
sed -i "s@neuvector/enforcer:${VERSION}@${DockerHub}/neuvector/enforcer:${VERSION}@g" ${DestPath}
