...
def set_payload(asset_amount = 0.0, #单价
    ...
    payload = {
        ...
        "purchaseDate": convert_date_to_short_string(purchase_date),
...
if __name__ == "__main__":
    ...
    input_excle_file_name = "server_list.xlsx"
    output_excel_file_dir = "/home/ubuntu/Downloads"  
    output_excel_file_name = "donate_server.xlsx"
    column_to_search = 0             # 可以是列名(如"姓名")或列索引(从0开始)
    search_keyword = "CPU"           # 要搜索的关键字
    sheet_name = "岩山赠与服务器"      # 查找的表
    ...
    for index, row in asset_data.iterrows():
        ...
        if WAY == PURCHASE:
            ...
            print(f"资产名称: {row['资产名称']},  型号: {row['型号']},  购入或入账日期: {row['登陆日期']}, S/N号: {row['S/N号']}, 资产编号: {row['资产编号']}")
            payload = set_payload(asset_amount = 0.0, 
                                  asset_code = row['资产编号'], 
                                  asset_name = row['资产名称'], 
                                  asset_type = 3, 
